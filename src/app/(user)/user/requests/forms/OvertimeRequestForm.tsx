...
function Popover({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div
          className={`absolute z-50 mt-2 min-w-[260px] rounded-2xl border bg-white p-2 shadow-lg ${alignClass}`}
        >
          {children}
        </div>
      )}
      ...
      >
        <DayPicker
          ...
          classNames={{
            ...
            day_selected:
              "bg-[#00156B] text-white hover:bg-[#00156B] hover:text-white",
...
export function OvertimeRequestForm({
  ...
  const previewUrl = useMemo(() => {
    ...
    return form.lampiran.type.startsWith("image/")
      ? URL.createObjectURL(form.lampiran)
      : null;
  }, [form.lampiran]);
  ...
  return (
    ...
    >
      ...
      >
        Duration:{" "}
        <span className="font-semibold text-gray-800">{durationHours}</span>{" "}
        hours
      </div>
