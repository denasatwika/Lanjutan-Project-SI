...
export default function Page(){
  ...
  useEffect(() => {
    ...
    attendanceApi.getTodayStatus()
      .then(data => {
        setStatus({ hasCheckedIn: data.hasCheckedIn, hasCheckedOut: data.hasCheckedOut })
      })
      .catch(err => {
  ...
  const handleCheckIn = async (photoData?: string) => {
    ...
    try {
      ...
      const result = await attendanceApi.checkIn(photoData)
  ...
  const handleCheckOut = async (photoData?: string) => {
    ...
    setLoading(true)
    try {
      await attendanceApi.checkOut(photoData)
      toast.success('Check-out berhasil!')
      setStatus({ hasCheckedIn: true, hasCheckedOut: true })
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Check-out gagal'
      toast.error(message)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded border bg-yellow-50 p-3 text-xs">
        ...
        <div>Status: hasCheckedIn={String(status?.hasCheckedIn)}, hasCheckedOut={String(status?.hasCheckedOut)}</div>
        <div>Loading: {String(loading)}</div>
      </div>

      {alreadyDone ? (
        <div className="rounded-2xl border bg-white p-6 text-center">
          <p className="text-gray-600">Anda sudah check-in dan check-out hari ini.</p>
        </div>
      ) : (
        <CameraCapture
