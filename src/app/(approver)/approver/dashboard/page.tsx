...
export default function ApproverDashboard() {
  ...
  // Fetch pending requests from backend (optimized single query)
  useEffect(() => {
    let cancelled = false;
  ...
  const absen = Math.max(
    0,
    totalKaryawan - (mode === "today" ? MOCK.today.hadir + MOCK.today.izin : 0),
  );
    ...
    <main className="mx-auto w-full max-w-[640px] p-3 pb-28">
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm/5 opacity-90">
              There are <span className="font-semibold">{pending}</span> request
        ...
        <div className="mt-2 inline-flex rounded-full bg-white/10 p-1">
          <ModeChip active={mode === "today"} onClick={() => setMode("today")}>
            Today
          </ModeChip>
      ...
      <section className="mt-5">
        <div className="mb-2 flex items-center justify-between px-1">
          ...
          <Link
            ...
            className="text-xs font-semibold text-[--B-800] underline underline-offset-2"
          >
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-2 shadow-sm">
          {loading ? (
            ...
            <p className="py-6 text-center text-xs text-slate-500">
              There are no request that needs your approval yet.
            </p>
          ) : (
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold">
                    {Math.round(MOCK.quality.onTimeRate * 100)}% ({" "}
                    {MOCK.today.hadir} / {MOCK.top.totalKaryawan} )
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs/5 text-slate-500">
                    Overtime rate this week:
                  </p>
                  <p className="text-sm font-semibold">
                    {MOCK.week.lemburJam} Hours
                  </p>
                </div>
              </div>
            </div>
            <div className="mt-3">
              <p className="mb-1 text-xs/5 text-slate-500">
                This Week's Night Owl
              </p>
        ...
        >
          <div className="pointer-events-auto rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-xl backdrop-blur">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm">
                <span className="font-semibold">{pending}</span> Request are
...
function ApprovalItem({
  ...
  return (
    ...
    >
      ...
      <Link
        href={`/approver/approval?request=${id}`}
        className="text-xs font-semibold text-[--B-800] underline underline-offset-2"
      >
        Review
      </Link>
