...
export default function ProfilePage() {
  ...
  const initialForm = useMemo<ProfileFormState>(() => {
    ...
    const resolvedDepartmentName =
      profileDepartmentName ?? userDepartment ?? "";
    ...
    const resolvedDepartmentId =
      profileDepartmentId ?? userDepartmentId ?? undefined;

    const userAddress =
      typeof user?.address === "string" && user.address.trim().length > 0
        ? user.address
        : undefined;
    const resolvedWallet = userAddress ?? "";

    const userAvatar =
      typeof user?.avatarUrl === "string" || user?.avatarUrl === null
        ? user.avatarUrl
        : undefined;
    const resolvedAvatar = userAvatar ?? null;
    ...
    getEmployee(user.id)
      .then(async (data) => {
        ...
        if (data.departmentId) {
          try {
            const deptName = await getDepartmentName(data.departmentId);
  ...
  async function saveProfile() {
    ...
    try {
      ...
      // Fetch department name if departmentId changed
      if (updated.departmentId) {
        try {
          const deptName = await getDepartmentName(updated.departmentId);
      ...
      auth.setUser({
        ...user,
        name:
          typeof updated.fullName === "string" ? updated.fullName : user.name,
        ...
        departmentId:
          typeof updated.departmentId === "string" &&
          updated.departmentId.length > 0
