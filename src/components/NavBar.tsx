    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-2 px-2 pb-4 border-b">
          <div
            className="size-8 rounded-full grid place-items-center font-bold text-white"
...
export function NavBar({ role: layoutRole }: { role: Role }) {
  ...
  const effectiveRoles =
    roles.length > 0 ? roles : primaryRole ? [primaryRole] : ["user"];
  const pathname = usePathname();
  ...
  return (
    <nav>
      {/* FIX DISINI:
         1. Hapus z-50 dari <nav> (karena tidak berguna di elemen static)
         2. Tambahkan z-[100] LANGSUNG di div yang 'fixed' ini.
      */}
      <div className="fixed inset-x-0 bottom-4 md:hidden z-[100]">
